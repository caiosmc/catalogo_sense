...
import { atualizarCategoriasNaURL, obterCategoriasDaURL } from "./urlFiltros"; // Caminho corrigido
...
function App() {
  ...
  useEffect(() => {
    const categoriasIniciais = obterCategoriasDaURL();
    if (categoriasIniciais.length > 0) {
      setCategoriasSelecionadas(categoriasIniciais);
    }
  }, []);
  ...
  const toggleCategoria = (cat) => {
    setCategoriasSelecionadas((prev) => {
      const novas = prev.includes(cat) ? prev.filter((c) => c !== cat) : [...prev, cat];
      atualizarCategoriasNaURL(novas);
      return novas;
    });
  };
